...
export const min = function min(c) {
  let colour;
  if (Array.isArray(c)) {
    colour = color({
      r: c[0],
  ...
  let alpha = colour.valpha;
  const rgb = colour.color;
  ...
  const word = colour.keyword();

  // the color lib returns the closest color, bail out of this
  if (color(word).hex() !== colour.hex()) {
